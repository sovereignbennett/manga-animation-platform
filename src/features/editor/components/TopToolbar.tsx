  ...
  return (
    <header className="h-14 shrink-0 flex items-center gap-3 px-3 border-b border-border bg-panel/60 backdrop-blur">
      ...
      <div className="flex items-center gap-1">
        <button className="tool-btn" title="Undo (⌘Z)" onClick={undo}>
          <Undo2 className="w-4 h-4" />
        </button>
        <button className="tool-btn" title="Redo (⌘⇧Z)" onClick={redo}>
          <Redo2 className="w-4 h-4" />
        </button>
      </div>

      <div className="flex-1 flex items-center justify-center">
        <div className="px-3 py-1.5 rounded-md bg-surface/60 border border-border text-xs text-muted-foreground">
          <span className="text-foreground/80 font-medium">{projectName}</span>
          <span className="ml-2 text-[10px] uppercase tracking-wider text-accent">● Autosaved</span>
        </div>
      </div>

      <button className="inline-flex items-center gap-2 h-9 px-3 rounded-md text-xs font-medium bg-gradient-to-r from-primary to-accent text-primary-foreground shadow-[0_0_24px_-6px_var(--primary-glow)] hover:brightness-110 transition">
