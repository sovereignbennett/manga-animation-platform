...
import { useEditor, type ImageLayer, type Layer } from "@/store/editorStore";

function isImageLayer(layer: Layer | undefined): layer is ImageLayer {
  ...
  const run = useCallback(
    async (layerId: string) => {
      const layer = useEditor
        .getState()
        .project.layers.filter(isImageLayer)
        .find((l) => l.id === layerId);
      if (!layer?.src) {
        setState({
          stage: "error",
      ...
      try {
        ...
        const cutouts = await Promise.all(
          result.parts.map(async (part) => {
            const { src: cutSrc, bounds } = await extractPartFromForeground(
              result.foreground!,
              part.bbox,
            );
            const pivotAbs = part.suggestedPivot ?? suggestPivot(part.kind, part.bbox);
            const anchor = pivotToAnchor(pivotAbs, bounds);
            return {
              partId: part.id,
              src: cutSrc,
              x: bounds.x,
        ...
        setState({
          stage: "done",
          progress: 1,
          message: `Cut into ${cutouts.length} parts`,
          result,
        });
      } catch (err) {
